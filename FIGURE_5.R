...
normalize_and_standardize <- function(df, method_name = "Unknown") {
  ...
  # Create a standardized dataframe with all cell types
  standardized_df <- data.frame(Mix = df$Mix)
  
  # Add each cell type column, using 0 if not present in original data
  ...
  # Process each row to ensure proper normalization
  for (i in 1:nrow(standardized_df)) {
    row_values <- as.numeric(standardized_df[i, -1])
    
    # Handle negative values and tiny scientific notation
...
# Convert to long format with consistent structure
...
for (mix_id in paste0("Mix", 1:6)) {
  mix_data <- all_data_long %>% 
    ...
    mutate(Method = factor(Method, levels = method_order),
           # Add spacing after Ground Truth
